...
PRODUCT_PAGES = {
    ...
    "live-studio": {"name": "ManPro AI - Live Studio", "category": "Live Production & Streaming", "image": "live-studio", "price": "₹4,999 / month", "note": "For professional live production teams", "lead": "Plan, produce and broadcast polished live experiences from one intelligent studio workspace.", "use_case": "Bring your cameras, scenes, guests, graphics and broadcast performance together for a dependable live-production workflow.", "features": ["Multi-camera live production control", "Scene, graphics and media management", "Remote guest and presenter coordination", "Live-stream analytics and recording library"], "highlights": [("Produce with confidence", "Keep every camera, cue and scene in one clear control room"), ("Broadcast anywhere", "Deliver professional live streams across your important channels"), ("Review every moment", "Keep recordings, performance insights and broadcast history ready")]} ,
}
...
PRODUCT_PAGES["live-studio"].update({"modules":[("Production Control Room", "Coordinate cameras, scenes, audio sources and production cues from one workspace."),("Live Stream Management", "Schedule, publish and monitor live broadcasts across your selected channels."),("Remote Guest Studio", "Bring presenters and guests into the production safely from anywhere."),("Graphics & Media", "Prepare lower thirds, media clips, brand scenes and on-screen graphics in advance."),("Recording Library", "Keep broadcast recordings organised for replay, review and post-production."),("Broadcast Analytics", "Review viewer engagement, stream health and performance after every event.")],"advantages":[("Professional output", "Create a polished, consistent live experience without scattered production tools."),("One calm control room", "Keep your crew aligned around scenes, guests, media and broadcast status."),("Flexible production", "Run webinars, events, interviews and product launches from the same platform."),("Built to scale", "Add channels, presenters and production workflows as your studio grows.")],"plans":[("Studio Start","₹4,999 / month","For compact live production teams",["One production workspace","Multi-camera scene control","Recording library","Basic stream analytics"]),("Studio Pro","₹9,999 / month","For regular live events and broadcasts",["Everything in Studio Start","Remote guest studio","Graphics and media control","Advanced analytics"]),("Enterprise","Custom","For high-volume production organisations",["Custom studio capacity","Multi-channel broadcasting","Tailored integrations","Dedicated production support"]) ]})
...
@app.get("/hdfoods/quotations/create")
def hdfoods_quotation_create(request: Request):
    if str(request.session.get("tenant_company_code") or "").upper() != "HDFOODS":
        return RedirectResponse("/customers", status_code=303)
    db = SessionLocal()
...
@app.post("/hdfoods/quotations/create/save")
async def hdfoods_quotation_create_save(request: Request):
    if str(request.session.get("tenant_company_code") or "").upper() != "HDFOODS":
        return RedirectResponse("/customers", status_code=303)
    form = await request.form()
    customer_id = form.get("customer_id", "")
    ...
    quotation_notes = form.get("quotation_notes", "")
    items_json = form.get("items", "[]")
    subtotal = form.get("subtotal", "0")
    discount_amount = form.get("discount_amount", "0")
    tax_amount = form.get("tax_amount", "0")
    grand_total = form.get("grand_total", "0")
    ...
    q_date = date.today().isoformat()
    seq = db.execute(text("SELECT COUNT(*) + 1 AS seq FROM hdfoods_quotations WHERE quotation_date = :q_date"), {"q_date": q_date}).scalar() or 1
    quotation_no = f"QTN-HDF-{date.today().strftime('%Y%m%d')}-{seq:04d}"

    db.execute(text("""
        INSERT INTO hdfoods_quotations
        (quotation_no, customer_id, customer_name, customer_mobile, quotation_date,
         total_amount, subtotal, discount_pct, discount_amount, tax_amount, validity_days, notes, items)
        VALUES (:quotation_no, :customer_id, :customer_name, :customer_mobile, :quotation_date,
                :grand_total, :subtotal, :discount_pct, :discount_amount, :tax_amount, :validity_days, :notes, :items)
    """), {
        "quotation_no": quotation_no, "customer_id": int(customer_id) if customer_id else None,
        "customer_name": customer_name, "customer_mobile": customer_mobile,
        "quotation_date": q_date, "grand_total": grand_total, "subtotal": subtotal,
        "discount_pct": float(discount_pct or 0), "discount_amount": discount_amount,
        "tax_amount": tax_amount, "validity_days": int(validity_days or 30),
        "notes": quotation_notes, "items": items_json,
    })
    db.commit()
    db.close()
    return RedirectResponse("/hdfoods/quotations", status_code=303)
